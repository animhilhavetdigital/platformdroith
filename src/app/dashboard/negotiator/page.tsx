...
import MesDossiersList from './MesDossiersList';
import {
...
import { CheckCircle, Clock, FolderOpen, MessageSquare, Users } from 'lucide-react';
    ...
    <DashboardLayout allowedRoles={['negotiator', 'super_admin']}>
      <div className="space-y-8">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white px-5 py-3 shadow-sm shadow-black/10">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm shadow-black/10">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              {mediationQueue.map((item) => (
                <div key={item.id} className="rounded-xl border border-gray-100 bg-gray-50 p-4 shadow-sm shadow-black/10">
...
function StatCard({ title, value, icon, color, border, bg }: { title: string; value: number; icon: React.ReactNode; color: string; border: string; bg: string }) {
  return (
    <div className={`rounded-2xl border ${border} ${bg} p-6 shadow-sm shadow-black/10 transition-all hover:-translate-y-0.5 hover:shadow-md`}>
