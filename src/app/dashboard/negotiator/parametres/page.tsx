...
import { User, Phone, Mail, Save, ShieldAlert } from 'lucide-react';
import { revalidatePath } from 'next/cache';

export default async function NegotiatorParametresPage() {
  const isPreview = isDevAccessEnabled();
  let profile: any = null;

  if (isPreview) {
    profile = devStore.profiles.find((p) => p.role === 'negotiator') || {
      prenom: 'Samir',
      nom: 'Bennani',
  ...
  async function handleUpdateProfile(formData: FormData) {
    ...
    if (isDevAccessEnabled()) {
      const negotiator = devStore.profiles.find((p) => p.role === 'negotiator');
      if (negotiator) {
        negotiator.prenom = prenom;
        negotiator.nom = nom;
    ...
    <DashboardLayout allowedRoles={['negotiator']}>
      <div className="max-w-2xl mx-auto space-y-8">
        <div>
          ...
          <p className="mt-1 text-gray-500">Gérez vos informations de négociateur et vos coordonnées professionnelles</p>
        </div>
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm shadow-black/10 space-y-6">
          <div className="flex items-center gap-3 border-b border-gray-50 pb-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Profil Négociateur</h2>
              <p className="text-xs text-gray-400">Coordonnées affichées sur vos rapports d&apos;intervention</p>
            </div>
          </div>
          ...
          <form action={handleUpdateProfile} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider">Téléphone Professionnel</label>
            ...
            <div className="space-y-2 opacity-75">
              ...
              <div className="relative">
                <input
                  type="email"
                  disabled
                  defaultValue={profile?.email || 'samir.bennani@example.com'}
            ...
            {isPreview && (
              <div className="rounded-xl bg-amber-50 p-4 flex gap-3 text-xs text-amber-800 items-start">
                ...
                <span>
                  Vous êtes en mode Aperçu (Demo). Les modifications sont sauvegardées temporairement en mémoire vive.
                </span>
              </div>
            )}
